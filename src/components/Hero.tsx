...
export const Hero: React.FC<HeroProps> = ({ onShopClick }) => {
  return (
    <section className="relative min-h-screen bg-brand-dark text-brand-light flex items-center justify-center overflow-hidden pt-20">
      ...
      {/* <div className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-[12%] -rotate-90 origin-center pointer-events-none select-none z-0 opacity-25 hidden lg:block">
        <img
          src={logo2}
          alt="axivers watermark"
          className="h-24 w-auto min-w-[125vh] object-contain transform scale-y-120"
        />
      </div> */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full z-10 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
        ...
        <div className="lg:col-span-7 space-y-8 text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            <button
              onClick={onShopClick}
              className="px-10 py-4 rounded-md bg-brand-light text-brand-dark font-display text-xs uppercase tracking-[0.05em] font-bold border border-brand-light hover:bg-transparent hover:text-brand-light transition-all duration-300 shadow-lg cursor-pointer"
            >
              Shop Collection
            </button>
            <a
