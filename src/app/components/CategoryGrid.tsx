...
export const CategoryGrid: React.FC = () => {
  ...
  return (
    <section id="categories" className="py-24 bg-[#09090b] text-[#ffffff] border-t border-zinc-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {categoryCards.map((cat) => (
            <Link
              ...
              className="group relative h-64 rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900 flex flex-col justify-end p-6 hover:border-white/50 transition-all duration-500 shadow-xl cursor-pointer"
            >
              {/* Background gradient overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/60 to-transparent z-10" />

              {/* Decorative accent line */}
              <div className="absolute top-4 right-4 text-white text-xs font-mono font-bold uppercase tracking-widest z-20 opacity-80 group-hover:opacity-100">
                Explore &rarr;
              </div>
              ...
              <div className="relative z-20 space-y-1">
                ...
                <h3 className="text-xl font-display uppercase tracking-wider font-bold text-white group-hover:text-zinc-300 transition-colors">
                  {cat.name}
                </h3>
